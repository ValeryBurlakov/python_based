...
# а затем может принимать дополнительные аргументы. Метод __new__ проверяет,
# существует ли уже экземпляр класса Archive (с использованием атрибута _instance).
...
# Если экземпляр еще не существует, метод создает новый экземпляр класса Archive с пустыми
# архивами для текстовых и числовых записей. В любом случае метод возвращает созданный
# или существующий экземпляр класса Archive.
# Метод __init__ - это конструктор экземпляра класса, который вызывается после
# создания экземпляра с использованием метода __new__. Метод __init__ принимает два аргумента:
...
# Эти записи могут быть затем добавлены в архив (списки archive_text и archive_number)
...
class Archive:
    ...
    def __new__(cls, string, num):
        if cls._instance is None:
            ...
            cls._instance.archive_text = [string]
            cls._instance.archive_number = [num]
        else:
            cls._instance.archive_text.append(string)
            cls._instance.archive_number.append(num)

        return cls._instance

    def __init__(self, string: str, num: int | float):
        self.string = string
        self.num = num

    def __str__(self):
        return f'Text is {self.string} and number is {self.num}. Also {self.archive_text} ' \
...
print(archive1.archive_text)  # Выведет: ['First Text']
